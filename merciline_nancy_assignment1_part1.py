# -*- coding: utf-8 -*-
"""MERCILINE_Nancy_Assignment1_Part1.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1tAzdHSJ0dWLcuVRpewIBuOetSykykCPV
...
def question_six(text):
    ...
    """
    # YOUR CODE HERE
    # Regex pattern for valid phone formats
    pattern = r'\b(\d{3})[-.\s](\d{3})[-.\s](\d{4})\b|\((\d{3})\)\s(\d{3})-(\d{4})'
    ...
    for match in matches:
        if match[0]:
            ...
            # Format: (XXX) XXX-XXXX
            results.append(f"{match[3]}-{match[4]}-{match[5]}")
...
def question_seven():
    ...
    for note in df:
        ...
        month, day, year = 1, 1, 1900   # default values
        ...
        # Month dd yyyy
            ...
            m = re.search(r'(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)[a-z]*\.?[- ,]+(\d{1,2}),?[- ]+(\d{4})', note, re.I)
            if m:
            ...
            # dd Month yyyy
                ...
                m = re.search(r'(\d{1,2})\s+(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)[a-z]*\.?,?\s+(\d{4})', note, re.I)
                if m:
                    day = int(m.group(1))
                ...
                # mm/yyyy
                    ...
                    # yyyy
                        ...
                        m = re.search(r'(19\d{2}|20\d{2})', note)
                        if m:
                            year = int(m.group(1))

        dates.append(pd.Timestamp(year=year, month=month, day=day))

    return pd.Series(dates).sort_values().index.to_series().reset_index(drop=True)

q7_result = question_seven()
print(len(q7_result))
print(q7_result.head())
